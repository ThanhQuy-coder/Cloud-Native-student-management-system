...
import { BarChart2, BookOpen, Building2, ClipboardList, Layers, LayoutDashboard, UserCheck, UserCog } from "lucide-react";
...
const MENUS: Record<string, { path: string; label: string; icon: React.ReactNode }[]> = {
  ...
  student: [
    { path: "/student/registration", label: "Đăng ký môn học", icon: <ClipboardList size={18} /> },
...
export default function Sidebar() {
  ...
  return (
    <div style={{ width: 240, minWidth: 240, height: "100vh", background: C.navy, display: "flex", flexDirection: "column", position: "fixed", top: 0, left: 0, zIndex: 100 }}>
      ...
      <div style={{ padding: "12px 20px 4px", fontSize: 10, fontWeight: 700, color: "rgba(255,255,255,0.35)", textTransform: "uppercase", letterSpacing: "0.1em" }}>
        {roleLabel}
      </div>
